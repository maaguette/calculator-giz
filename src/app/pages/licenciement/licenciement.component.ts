...
export class LicenciementComponent implements OnInit {
  ...
  process() {
    ...
    if (this.numberYear < 1) {
      ...
      if (compareDate1 >= compareDate2) {
        if (this.numberYear <= 3) {
          this.resultIndemnity = Math.round((this.numberYear * solde * 0.25) + ((this.numberMonth / 12) * solde * 0.25) + ((this.numberDay / 360) * solde * 0.25));
        }
        if (this.numberYear > 3 && this.numberYear <= 8) {
          this.resultIndemnity = Math.round((solde * 3 * 0.25) + (solde * (this.numberYear - 3) * 0.35) + (solde * (this.numberMonth / 12) * 0.35) + (solde * (this.numberDay / 360) * 0.35));
        }
        if (this.numberYear > 8) {
          this.resultIndemnity = Math.round((solde * 3 * 0.25) + (solde * 5 * 0.35) + (solde * (this.numberYear - 8) * 0.45) + (solde * (this.numberMonth / 12) * 0.45) + (solde * (this.numberDay / 360) * 0.45));
        }
      }
  ...
  processMore() {
    ...
    if (this.numberYearMore < 1) {
      ...
      if (compareDate1 < compareDate2) {
        if (this.numberYearMore <= 3) {
          this.resultIndemnityMore = Math.round(((this.numberYearMore * totalSolde * 0.25) + ((this.numberMonthMore / 12) * totalSolde * 0.25) + ((this.numberDayMore / 360) * totalSolde * 0.25)));
        }
        if (this.numberYearMore > 3 && this.numberYearMore <= 8) {
          this.resultIndemnityMore = Math.round((totalSolde * 3 * 0.25) + (totalSolde * (this.numberYearMore - 3) * 0.35) + (totalSolde * (this.numberMonthMore / 12) * 0.35) + (totalSolde * (this.numberDayMore / 360) * 0.35)
          );
        }
        if (this.numberYearMore > 8) {
          this.resultIndemnityMore = Math.round((totalSolde * 3 * 0.25) + (totalSolde * 5 * 0.35) + (totalSolde * (this.numberYearMore - 8) * 0.45) + (totalSolde * (this.numberMonthMore / 12) * 0.45) + (totalSolde * (this.numberDayMore / 360) * 0.45)
          );
        }
      }
      this.resultIndemnityMore = this.formatCurrencyNew(this.resultIndemnityMore);
