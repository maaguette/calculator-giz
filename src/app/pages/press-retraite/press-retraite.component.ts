...
export class PressRetraiteComponent implements OnInit {
  ...
  process(solde: any, engagementDate: any, retraiteDate: any) {
    ...
    let compareDate1 = new Date(this.utilService.formatDateSeparate(retraiteDate));
    ...
    if (this.numberYear < 1) {
      ...
      if (this.intervalOlder <= 5) {
        this.result = Math.round((this.numberYear * solde * 0.35) + ((this.numberMonth / 12) * solde * 0.35) + ((this.numberDay / 360) * solde * 0.35));
      }
      if (this.intervalOlder > 5 && this.intervalOlder <= 10) {
        this.result = Math.round((solde * 5 * 0.35) + (solde * (this.numberYear - 5) * 0.4) + (solde * (this.numberMonth / 12) * 0.4) + (solde * (this.numberDay / 360) * 0.4));
      }
      if (this.intervalOlder > 10 && this.intervalOlder <= 15) {
        this.result = Math.round((solde * 5 * 0.35) + (solde * 5 * 0.4) + (solde * (this.numberYear - 10) * 0.5) + (solde * (this.numberMonth / 12) * 0.5) + (solde * (this.numberDay / 360) * 0.5));
      }
      if (this.intervalOlder > 15) {
        this.result = Math.round((solde * 5 * 0.35) + (solde * 5 * 0.40) + (solde * 5 * 0.50) + (solde * (this.numberYear - 15) * 0.6) + (solde * (this.numberMonth / 12) * 0.6) + (solde * (this.numberDay / 360) * 0.6));
      }
  ...
  processMore() {
    ...
    if (solde1 == "" && solde2 == "" && solde3 == "" && solde4 == "" && solde5 == "" && solde6 == "" && solde7 == "" && solde8 == "" && solde9 == "" && solde10 == "" && solde11 == "" && solde12 == "") {
      ...
      this.resultIndemnityMore = this.process(totalSolde, this.ngPickerEngagementDateMore, this.retraiteDateMore)
